...
from utils import get_model, get_model_path, load_data
...
LLAMA8B_ENCODE_MODE_ENV = "PRAG_LLAMA8B_ENCODE_MODE"


def use_llama8b_single_gpu_encode(args) -> bool:
    return (
        args.model_name == "llama3-8b-instruct"
        and os.environ.get(LLAMA8B_ENCODE_MODE_ENV) == "single_gpu"
    )
...
def train(question, augments, args, model, tokenizer, 
    ...
    max_length = 3000
    if use_llama8b_single_gpu_encode(args):
        max_length = min(max(len(input_ids) for input_ids in prompt_ids), max_length)
    train_data = TrainingData(prompt_ids, tokenizer, max_length=max_length)
